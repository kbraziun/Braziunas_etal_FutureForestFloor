...
clim.in <- read.csv("processed_data/iland_clim_soils/future_predictors/sdm_predictor_set_climate_10m.csv") %>%
...
pred.listin <- read.csv("processed_data/iland_sdm_predictions/prediction_scenarios.csv") %>%
...
for(i in 1:dim(pred.list)[1]) {
  ...
  detectCores() # 8
  cl = makeCluster(4) # use 3, trying 4
  # more on server, 40
  # cl = makeCluster(10)
  ...
  foreach(h=unique(iland.pred$chunk),
    ...
    turn.prep <- read.csv(paste0("processed_data/iland_sdm_predictions/historical_predictions_10m/sdm_predictions_species_10m_chunk_",h,".csv")) %>%
    ...
    ### write out
    # species sums
    write.csv(species.out, paste0("processed_data/iland_sdm_predictions/future_predictions_10m/",pred.list[i,]$pred_forest,"Forest_",pred.list[i,]$pred_climate,"Climate_",pred.list[i,]$pred_gcm,"_",pred.list[i,]$pred_wind,"_",pred.list[i,]$rep,"_",pred.list[i,]$pred_decade,"_speciesSums_chunk_",h,".csv"),row.names=FALSE)
    
    # plots
    write.csv(plots.out, paste0("processed_data/iland_sdm_predictions/future_predictions_10m/",pred.list[i,]$pred_forest,"Forest_",pred.list[i,]$pred_climate,"Climate_",pred.list[i,]$pred_gcm,"_",pred.list[i,]$pred_wind,"_",pred.list[i,]$rep,"_",pred.list[i,]$pred_decade,"_plot_chunk_",h,".csv"),row.names=FALSE)
...
# continues to use pred.list from above
...
for(i in 1:dim(pred.list)[1]) {
  ...
  for(j in unique(chunk.list$chunk)) {
    ...
    # read in chunk
    resp.in <- read.csv(paste0("processed_data/iland_sdm_predictions/future_predictions_10m/",pred.list[i,]$pred_forest,"Forest_",pred.list[i,]$pred_climate,"Climate_",pred.list[i,]$pred_gcm,"_",pred.list[i,]$pred_wind,"_",pred.list[i,]$rep,"_",pred.list[i,]$pred_decade,"_plot_chunk_",j,".csv")) %>%
  ...
  resp.mean <- cbind(pred.list[i,],data.frame(mean_richness = mean(resp.out$corr_richness),
...
# combine with cover and write out
cover.meanout %>%
  left_join(resp.meanout, by=c("pred_decade","pred_forest","pred_climate","pred_gcm","pred_wind","pred_reps","forest_climate","rep")) %>%
  write.csv("processed_data/iland_sdm_predictions/future_prediction_summaries/scenario_mean_cv.csv",row.names=FALSE)
...
for(j in unique(chunk.list$chunk)) {
  ...
  resp.sp <- resp.out %>%
    ...
    group_by(forest_climate,pred_decade,id_model,name) %>%
    ...
    dplyr::select(c(forest_climate:cv_turnover,light_cold,light_indiff,light_warm,shade_cold,shade_indiff,shade_warm,fern,graminoid,herb,shrub,lost,new,retained))
  
  # change and variability by scenario
  resp.sub <- resp.out %>%
    pivot_longer(c(light_cold:retained)) %>%
    group_by(forest_climate,pred_decade,pred_gcm,pred_wind,id_model,name) %>%
    summarise(mean_richness = mean(corr_richness),
              cv_richness = (sd(corr_richness)/mean_richness) * 100,
              mean_T = mean(corr_T),
              cv_T = (sd(corr_T)/mean_T)*100,
...
# write out
write.csv(resp.spout, "processed_data/iland_sdm_predictions/future_prediction_summaries/spatial_response_change.csv",row.names=FALSE)
write.csv(resp.subout, "processed_data/iland_sdm_predictions/future_prediction_summaries/spatial_response_change_scenarios.csv",row.names=FALSE)
...
hx.in <- list.files("processed_data/iland_sdm_predictions/historical_predictions_10m/", pattern="_plot_", full.names = TRUE) %>%
  map_df(~read_csv(.)) %>%
  mutate(decade="2020",scen="all") %>%
...
resp.spin <- read.csv("processed_data/iland_sdm_predictions/future_prediction_summaries/spatial_response_change.csv") %>%
...
cover.subin <- read.csv("processed_data/iland_sdm_predictions/future_prediction_summaries/spatial_cover_change_scenarios.csv") %>%
  rename(cover=cover_mean,
         decade=pred_decade) %>%
  mutate(scen=paste0("wind_",pred_wind)) %>%
...
for(i in unique(resp.all$response)) {
  print(i)
  name.in <- i 
...
writeRaster(rast.out[[-1]], filename=paste0("processed_data/iland_sdm_predictions/response_rasters.tif"),overwrite=TRUE)
...
spsums.hx <- setNames(
  list.files("processed_data/iland_sdm_predictions/historical_predictions_10m/", pattern="speciesSums", full.names=TRUE), 
  list.files("processed_data/iland_sdm_predictions/historical_predictions_10m/", pattern="speciesSums")) %>%
  ...
  mutate(pred_decade="2020",
         pred_gcm=NA,pred_wind=NA,pred_rep=NA,lost=NA)
summary(spsums.hx)

# read in species sums for each chunk and scenario
spsums.in <- setNames(
  list.files("processed_data/iland_sdm_predictions/future_predictions_10m/", pattern="speciesSums", full.names=TRUE), 
  list.files("processed_data/iland_sdm_predictions/future_predictions_10m/", pattern="speciesSums")) %>%
...
spsums.agg <- spsums.in %>%
  ...
  group_by(pred_decade,pred_gcm,pred_wind,pred_rep,species_name) %>%
...
for(i in 1:dim(pred.list)[1]) {
  ...
  write.csv(plots.out, paste0("processed_data/iland_sdm_predictions/rand_sample_predictions/",pred.list[i,]$pred_forest,"Forest_",pred.list[i,]$pred_climate,"Climate_",pred.list[i,]$pred_gcm,"_",pred.list[i,]$pred_wind,"_",pred.list[i,]$rep,"_",pred.list[i,]$pred_decade,"_plot.csv"),row.names=FALSE)
