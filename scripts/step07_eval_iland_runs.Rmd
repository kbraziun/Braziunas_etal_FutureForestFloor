...
beetle.in %>%
  ggplot(aes(x=year,y=infestedArea_ha, group=rep, color=rep, fill=rep)) +
...
wind.complete %>%
  ggplot(aes(x=year,y=area_ha, group=rep, color=rep, fill=rep)) +
  facet_grid(gcm~wind_speed) +
  geom_line() +
  theme_bw() # check that reps are different
...
ba.prep <- land.in %>%
  ...
  filter(year==80) %>%
  group_by(year) %>%
  summarise(min=min(basal_area_m2),max=max(basal_area_m2))
